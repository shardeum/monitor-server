...
async function getListOfFoundationNodes(): Promise<unknown[]> {
  /*
    ...
    is invoked.
     */
  const getFoundationNodes = async (
    url: string
  ): Promise<{data: {nodes: any[]}}> => {
    let res = {
      data: {
        nodes: [],
      },
  ...
  try {
    const results = await Promise.all([
      getFoundationNodes(
        'https://storage.googleapis.com/shardeum-info-and-counts/network/itn4/nodeInfo.json'
      ),
      getFoundationNodes(
        'https://storage.googleapis.com/shardeum-info-and-counts/network/bb/nodeInfo.json'
      ),
      getFoundationNodes(
        'https://storage.googleapis.com/shardeum-info-and-counts/network/dev-us/nodeInfo.json'
      ),
      getFoundationNodes(
        'https://storage.googleapis.com/shardeum-info-and-counts/network/dev-apac/nodeInfo.json'
      ),
    ]);
...
export const listFoundationNodes = (
  req: RequestWithBody,
  res: Response,
  next: NextFunction
) => {
