...
export class Node {
    ...
    return new Promise((resolve, reject) => {
      ...
      .catch((err) => {
        Logger.mainLogger.error(
          'FAILED TO GET ARCHIVER CYCLE RECORD. ' + (this.cycleRecordCounter < 0)
      ...
      axios
        ...
        .then((res) => {
          if (res.status !== 200) {
            ...
            } else if (nodeInfo.status === 'standby' && !this.nodes.standby[pk]) {
  ...
  joining(publicKey: string, nodeIpInfo: NodeIpInfo, appData: NodeInfoAppData): void {
    if (config.allowBogon === false) {
      if (isBogonIP(nodeIpInfo.externalIp)) {
        ...
        if (config.verboseLog)
          Logger.mainLogger.info(
            `Received bogon ip at joining report. public key: ${publicKey}, nodeIpInfo: ${JSON.stringify(nodeIpInfo)}`
          )
        return
      }
    } else {
      ...
      if (isInvalidIP(nodeIpInfo.externalIp)) {
        ...
        if (config.verboseLog)
          Logger.mainLogger.info(
            `Received invalid ip at joining report. public key: ${publicKey}, nodeIpInfo: ${JSON.stringify(nodeIpInfo)}`
          )
        return
      }
    }
    ...
    if (existingStandbyNodePublicKey) {
      ...
      Logger.mainLogger.info('Joining node is found in the standby list. Removing existing standby node')
    }
    ...
    if (existingSyncingNode) {
      ...
      Logger.mainLogger.info('Joining node is found in the syncing list. Removing existing syncing node.')
    }
    ...
    if (existingActiveNode) {
      ...
      Logger.mainLogger.info('Joining node is found in the active list. Removing existing active node.')
    }
  }

  getExistingActiveNode(nodeId: string, nodeIpInfo: NodeIpInfo): ActiveReport {
    if (config.verboseLog) Logger.mainLogger.debug('Checking existing active node.', nodeId, nodeIpInfo)
    try {
  ...
  getExistingSyncingNode(nodeId: string, nodeIpInfo: NodeIpInfo): SyncReport {
    if (config.verboseLog) Logger.mainLogger.debug('Checking existing syncing node.', nodeId, nodeIpInfo)
    try {
  ...
  getExistingStandbyNode(publicKey: string, nodeIpInfo: NodeIpInfo): string {
    if (config.verboseLog) Logger.mainLogger.debug('Checking existing standby node.', publicKey, nodeIpInfo)
    try {
      if (this.nodes.joining[publicKey]) {
        Logger.mainLogger.debug('Found existing standby node with same publicKey', publicKey)
        return publicKey
      } else {
    ...
    try {
      if (config.allowBogon === false) {
        if (isBogonIP(nodeIpInfo.externalIp)) {
          ...
          if (config.verboseLog)
            Logger.mainLogger.info(
              `Received bogon ip at joined report. public key: ${publicKey}, nodeId: ${nodeId}, nodeIpInfo: ${JSON.stringify(
                nodeIpInfo
              )}`
            )
          return
        ...
        if (isInvalidIP(nodeIpInfo.externalIp)) {
          ...
          Logger.mainLogger.info(
            `Received invalid ip at joined report. public key: ${publicKey}, nodeId: ${nodeId}, nodeIpInfo: ${JSON.stringify(
              nodeIpInfo
            )}`
          )
          return
      ...
      Logger.historyLogger.info(`joined ${nodeId} ${nodeIpInfo.externalIp} ${nodeIpInfo.externalPort} ${this.counter}`)
    } catch (e) {
  ...
  async heartbeat(nodeId: string, data: ActiveReport): Promise<void> {
    if (config.verboseLog)
      Logger.mainLogger.info(`Running heartbeat for nodeId: ${nodeId}, data: ${JSON.stringify(data)}`)
    ...
    try {
      if (config.allowBogon === false) {
        if (isBogonIP(data.nodeIpInfo.externalIp)) {
          ...
          if (config.verboseLog)
            Logger.mainLogger.info(
              `Received bogon ip at heartbeat data. nodeId: ${nodeId}, nodeIpInfo: ${JSON.stringify(
                data.nodeIpInfo.externalIp
              )}`
            )
          return
        }
        ...
        if (isInvalidIP(data.nodeIpInfo.externalIp)) {
          ...
          Logger.mainLogger.info(
            `Received invalid ip at heartbeat data. nodeId: ${nodeId}, nodeIpInfo: ${JSON.stringify(
              data.nodeIpInfo.externalIp
            )}`
          )
          return
  ...
  updateAvgAndMaxTps() {
    ...
    try {
      ...
      const newAvgTps = Math.round((this.totalProcessed - this.lastTotalProcessed) / (this.reportInterval / 1000))
  ...
  logSummaryToConsole() {
    ...
    console.log(
      `Standby: ${Object.keys(this.nodes.joining).length}, syncing: ${
        Object.keys(this.nodes.syncing).length
      }, active: ${Object.keys(this.nodes.active).length}`
    )
  }
  ...
  updateRejectedTps() {
    try {
      ...
      const rejectedTps = Math.round((this.totalTxRejected - this.lastTotalTxRejected) / (this.reportInterval / 1000))
  ...
  getAppVersions() {
      ...
      if (aggregatedAppVersion.has(appData.shardeumVersion)) {
        ...
        aggregatedAppVersion
          ...
          .cliVersions.set(
            ...
            aggregatedAppVersion.get(appData.shardeumVersion).cliVersions.get(appData.operatorCLIVersion) + 1
          )
        ...
        aggregatedAppVersion
          ...
          .guiVersions.set(
            ...
            aggregatedAppVersion.get(appData.shardeumVersion).guiVersions.get(appData.operatorGUIVersion) + 1
          )
    ...
    for (const nodeId in this.nodes.joining) {
      ...
      if (!appData) {
        Logger.mainLogger.error(`Unable to find appData for joining node ${nodeId}`)
        continue
      }
      ...
      if (aggregatedAppVersion.has(appData.shardeumVersion)) {
        ...
        aggregatedAppVersion.get(appData.shardeumVersion).joiningNodeCount =
          (aggregatedAppVersion.get(appData.shardeumVersion).joiningNodeCount || 0) + 1
    ...
    for (const nodeId in this.nodes.syncing) {
      ...
      if (!appData) {
        Logger.mainLogger.error(`Unable to find appData for syncing node ${nodeId}`)
        continue
      }
      ...
      if (aggregatedAppVersion.has(appData.shardeumVersion)) {
        ...
        aggregatedAppVersion.get(appData.shardeumVersion).syncingNodeCount =
          (aggregatedAppVersion.get(appData.shardeumVersion).syncingNodeCount || 0) + 1
  ...
  resetRareCounters() {
    ...
    for (const nodeId in this.nodes.active) {
      try {
        ...
        axios.get(`httP://${nodeIpInfo.externalIp}:${nodeIpInfo.externalPort}/rare-counts-reset`)
      } catch (e) {
        Logger.errorLogger.error('Unable to reset rare counters for', nodeId, e)
      }
    }
  }
        ...
        for (const key in counterMap) {
          ...
          for (const key2 in subCounterMap) {
            if (aggregatedCounter[key].subCounters[key2])
              aggregatedCounter[key].subCounters[key2].count += subCounterMap[key2].count
            else
