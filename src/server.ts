...
import MemoryModule, { memoryReportingInstance } from './class/profiler/MemoryReporting'
...
statistics.on('snapshot', memoryReportingInstance.updateCpuPercent.bind(memoryReportingInstance))
  ...
  try {
    ...
    function getSortedNodeLinks(nodes, sortOrder) {
      let nodesArray = nodes.map((node) => {
        ...
        const port =
          node && node.nodeIpInfo && node.nodeIpInfo.externalPort ? node.nodeIpInfo.externalPort : 'NoExternalPort'
