...
function validateReport(arg) {
  ...
  if (!isExist(arg.repairsFinished) || typeof arg.repairsFinished !== 'number') {
  ...
  if (arg.nodeIpInfo) {
    ...
    if (!arg.nodeIpInfo.externalPort || typeof arg.nodeIpInfo.externalPort !== 'number') {
      console.log(`No externalPort`)
      return false
    }
    if (!arg.nodeIpInfo.internalIp || typeof arg.nodeIpInfo.internalIp !== 'string') {
      console.log(`No internalIp`)
      return false
    }
    if (!arg.nodeIpInfo.internalPort || typeof arg.nodeIpInfo.internalPort !== 'number') {
  ...
  if (!isExist(arg.partitionsCovered) || typeof arg.partitionsCovered !== 'number') {
